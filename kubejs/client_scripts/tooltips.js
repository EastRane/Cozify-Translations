ItemEvents.tooltip(e => {
    ...
    'При установке появится вырезанная тыква.') 
    ...
    'Сырая еда/ингредиент (Raw food/ingredient)')
    ...
    'Приготовленная еда/блюдо (Cooked food/meal)')
    ...
    'Лампа/Фонарь/Свет/Освещение (Lamp/Lantern/Light/Glows)')
    ...
    'Висящие растения! (Hanging Plants!)')
    ...
    'Растительные ковры! (Plant Carpets!)')
    ...
    'Блоки профессий! (Job Blocks!)')

	e.add('reaping:iron_reaping_tool', 
    'Базовый шанс неудачи: 45%'
    )

	e.add('reaping:gold_reaping_tool', 
    'Базовый шанс неудачи: 34%'
    )

	e.add('reaping:diamond_reaping_tool', 
    'Базовый шанс неудачи: 18%'
    )

	e.add('reaping:netherite_reaping_tool', 
    'Базовый шанс неудачи: 9%'
    )


	e.add('hexerei:moon_dust_brush', 
    'Быстрый полёт может вызвать проблемы с производительностью сервера. Пожалуйста, летайте с учётом других игроков.'
    )

	e.add('alexsmobs:shattered_dimensional_carver', 'Не используйте! Этот предмет содержит баги. Подробности смотрите в заданиях.'
    )

	e.add('alexsmobs:warped_muscle', 'Можно получить через Врата Нижнего мира.'
    )

	e.add('alexsmobs:hemolymph_sac', 'Можно получить через Врата Нижнего мира.'
    )

	e.add('sophisticatedbackpacks:backpack', 'На этой сборке нельзя скрафтить. Ищите в сундуках Городов Края!'
    )

	e.add('domesticationinnovation:wayward_lantern', 'Используйте осторожно! При приближении к блуждающему фонарю все прирученные мобы телепортируются к нему.'
    )
...
// Mob Lassos
    ...
    e.add(['moblassos:golden_lasso', 
        ...
        `\u00A7d`+'Помещение временных лассо в инвентарь может вызвать сильные лаги, так как эти сущности будут постоянно пытаться сбежать.'
    )

    e.add('brewinandchewin:keg',`\u00A7d`+'Предупреждение: соленья, вяленое мясо и некоторые рецепты могут вызывать краши.')

    e.add('#forge:maj_acs', `\u00A7d`+'Не комбинируйте эти предметы в инвентаре - это может вызвать краш!')

	e.add('kubejs:pink_moon', 'Розовая Луна появляется в апреле.')
	e.add('kubejs:flower_moon', 'Цветочная Луна появляется в мае.')
	e.add('kubejs:strawberry_moon', 'Клубничная Луна появляется в июне.')
	e.add('kubejs:antler_moon', 'Оленья Луна появляется в июле.')
	e.add('kubejs:harvest_moon', 'Урожайная Луна появляется в августе.')
	e.add('kubejs:corn_moon', 'Кукурузная Луна появляется в сентябре.')
	e.add('kubejs:oak_moon', 'Дубовая Луна появляется в декабре.')
	e.add('kubejs:hunter_moon', 'Лунная Охота появляется в октябре.')
	e.add('kubejs:frost_moon', 'Морозная Луна появляется в ноябре.')
	e.add('kubejs:wolf_moon', 'Волчья Луна появляется в январе.')

    e.add('#forge:moon_certificates', `\u00A7d`+'Автор арта: LoFi Fruit')
...
// Scythes (egg and head drops)
    e.add(
        ...
        "Мобы, убитые этим оружием, имеют шанс выпадения особой головы или яйца призыва!"
    );

    e.add(['contact:wrapping_paper','contact:ender_wrapping_paper'], 'ПКМ для открытия интерфейса')
})
